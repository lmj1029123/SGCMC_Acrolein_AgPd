...
# We only desorb when the atom in first layer is changed 
desorb_chance = 1

n_total = np.prod(size)
slab = fcc111('Ag',size=size, a=4.163)

slab.center(vacuum=6, axis=2)

Pd_ids = np.random.choice(size[0]*size[1]*2, n_Pd, replace = False) + size[0]*size[1]*(size[2] - 2)
for i in Pd_ids:
	slab[i].symbol = 'Pd'
...
sl = site_locator(deepcopy(slab), h, size, cutoff, site_distance, site_model_dir, max_N_acrolein, m_element='Pd')
t1 = time.time()
...
occupied_dict= {}
N_acrolein, occupied_sites, occupied_dict = sl.allocate_sites(site_energies, all_active_sites, occupied_sites, occupied_dict)
t4 = time.time()
sl.update_sites(occupied_sites, occupied_dict, N_acrolein)

E0 = np.sum([occupied_dict[str(int(s))] for s in occupied_sites]) + cal_energy(slab_model_dir, slab)
E = [E0]

nl_bare1 = NeighborList([cutoff/2]*len(slab),
					skin=0.0, bothways=True,
					self_interaction=True,
					primitive=NewPrimitiveNeighborList)
nl_bare1.update(slab)

nl_bare2 = NeighborList([cutoff]*len(slab),
					skin=0.0, bothways=True,
					self_interaction=True,
					primitive=NewPrimitiveNeighborList)
...
for epo in range(epochs):
	...
	if ((ind >= (np.prod(size) - np.prod(size[:2]))) and (np.random.uniform(0,1,1) <= desorb_chance)):
